SUBROUTINE init(pcorr)  
  ...
  !      real*8 ::lat(0:NI+1,0:NJ+1),lon(0:NI+1,0:NJ+1)           
  !      real*8 ::lat(0:NJ+1)                                     
  REAL*8 ::  xdu(0:NI+1,0:NJ+1),ydu(0:NI+1,0:NJ+1),                       &
...
!     END DO
     ...
     DO j =1, NJ
        ffi(:,j)= fconst*dSin(latrad(j)) 
        bbi(:,j)= fnhhy*fconst*dCos(latrad(j)) 
     ENDDO
     DO j =0, NJ
        ffj(:,j)= fconst*dSin(0.5d0*(latrad(j+1)+latrad(j))) 
        bbj(:,j)= fnhhy*fconst*dCos(0.5d0*(latrad(j+1)+latrad(j))) 
     ENDDO
     DO j=0, NJ+1
        ffc(:,j)= fconst*dSin(latrad(j)) 
        bbc(:,j)= fnhhy*fconst*dCos(latrad(j)) 
     ENDDO
     !                                                                       
     !     Initialize s,T                                                    
