MODULE particles
  USE header, ONLY : NI,NJ,NK,uf,vf,wf,Jifc,Jjfc,J2d,ux,vy,NPR,wz,PI,dtf,vor,shear,rho,strain,zf,s,parti_file_num,DL
  ...
  SUBROUTINE get_parti_vel(time)
    ...
    DO ip = 1, NPR
       ...
       IF (parti(ip)%j < NJ .AND. parti(ip)%j > 0 .AND. &
          ...
          CALL sigma2z(parti(ip)%i,parti(ip)%j+0.5d0,dfloat(kc),swap2)
          CALL sigma2z(parti(ip)%i,parti(ip)%j+0.5d0,dfloat(kc+1),swap3)
          ...
          CALL sigma2z(parti(ip)%i+0.5d0,parti(ip)%j,dfloat(kc),swap2)
          CALL sigma2z(parti(ip)%i+0.5d0,parti(ip)%j,dfloat(kc+1),swap3)
          dz = (swap1 - swap2) / ( swap3 - swap2 )
          ...
          CALL sigma2z(parti(ip)%i,parti(ip)%j,dfloat(kfc),swap2)
          CALL sigma2z(parti(ip)%i,parti(ip)%j,dfloat(kfc+1),swap3)
          dz = (swap1 - swap2) / ( swap3 - swap2 )
          parti(ip)%z=swap1*DL
          ...
          CALL sigma2z(parti(ip)%i+0.5d0,parti(ip)%j+0.5d0,dfloat(kc),swap2)
          CALL sigma2z(parti(ip)%i+0.5d0,parti(ip)%j+0.5d0,dfloat(kc+1),swap3)
          dz = (swap1 - swap2) / ( swap3 - swap2 )
