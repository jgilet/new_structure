...
!     If the BCs are changing with time we must modify this subroutine. 
      IMPLICIT NONE 
      integer i,j,k 
      double precision dtloc,edt,dpdxi,dpdeta,dpdsig,pxi,peta,psig,     &
     &     pf(0:NI+1,0:NJ+1,0:NK+1),gin                                 
!                                                                       
      if (dtloc.gt.1.d-16) then 
         edt= EPS/dtloc 
      else 
...
!     --------                                                          
      ...
      do i=1,NI 
         do j=1,NJ 
            ...
            dpdxi= 0.25*( pf(i+1,j,k+1) +pf(i+1,j,k)-pf(i-1,j,k+1) -pf(i-1,j,k) )                          
            dpdeta= 0.25*( pf(i,j+1,k+1) +pf(i,j+1,k)-pf(i,j-1,k+1) -pf(i,j-1,k) )                          
            psig= gin*(-skfc(i,j,k)- gqk(i,j,k,1)*dpdxi - gqk(i,j,k,2)*dpdeta & 
...
!     bottom edges                                                      
      ...
      do i=1,NI 
         ...
         dpdxi= 0.25*( pf(i+1,j,k+1) +pf(i+1,j,k)-pf(i-1,j,k+1) -pf(i-1,j,k) )                             
         dpdeta= 0.5*( pf(i,j+1,k+1) +pf(i,j+1,k)-pf(i,j,k+1) -pf(i,j,k) )        
         psig= gin*(-skfc(i,j,k)- gqk(i,j,k,1)*dpdxi - gqk(i,j,k,2)*dpdeta )              
         pf(i,j,0)= pf(i,j,1) - psig 
      end do 
      j=NJ+1 
      do i=1,NI 
         gin= 1.d0/gqk(i,j,0,3) 
         dpdxi= 0.25*( pf(i+1,j,k+1) +pf(i+1,j,k)-pf(i-1,j,k+1) -pf(i-1,j,k) )                             
         dpdeta= 0.5*( pf(i,j,k+1) +pf(i,j,k)-pf(i,j-1,k+1) -pf(i,j-1,k) )              
         psig= gin*(-skfc(i,j,k)- gqk(i,j,k,1)*dpdxi - gqk(i,j,k,2)*dpdeta )              
         pf(i,j,0)= pf(i,j,1) - psig 
      end do 
!                                                                       
...
!     --------------                                                    
      ...
      do i=1,NI 
         do k=1,NK 
            ...
            dpdxi= 0.25*( pf(i+1,j+1,k) +pf(i+1,j,k)-pf(i-1,j+1,k) -pf(i-1,j,k) )                          
            dpdsig= 0.25*( pf(i,j+1,k+1) +pf(i,j,k+1)-pf(i,j+1,k-1) -pf(i,j,k-1) )                          
            peta= gin*(- gqj(i,j,k,1)*dpdxi - gqj3(i,j,k)*dpdsig+(edt*(cyf(i,j,k)-vfbcs(i,k))-sjfc(i,j,k)) )           
            !c     &  - gqj(i,j,k,1)*dpdxi                                 
...
!                                                                       
      ...
      do i=1,NI 
         do k=1,NK 
            ...
            dpdxi= 0.25*( pf(i+1,j+1,k) +pf(i+1,j,k)-pf(i-1,j+1,k) -pf(i-1,j,k) )                          
            dpdsig= 0.25*( pf(i,j+1,k+1) +pf(i,j,k+1)-pf(i,j+1,k-1) -pf(i,j,k-1) )                          
            peta= gin*(- gqj(i,j,k,1)*dpdxi - gqj3(i,j,k)*dpdsig+(edt*(cyf(i,j,k)-vfbcn(i,k))-sjfc(i,j,k)) )           
            !c     &           - gqj(i,j,k,1)*dpdxi                                 
